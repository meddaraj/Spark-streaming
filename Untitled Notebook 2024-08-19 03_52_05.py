...
sys.argv = ['script_name.py', '--output', 'LOG', '--num', '10', '--sleep', '2']
...
#outFileName = 'C:\\Users\\Raj\\Desktop\\Kaam_ka_cheez\\Fake-Apache-Log-Generator-master\\LogFiles\\StreamTest\\access_log_'+timestr+'.log' if not file_prefix else file_prefix+'access_log.log'

outFileName = '/dbfs/FileStore/faker_data/access_log.log' if not file_prefix else file_prefix+'access_log.log'
...
while (flag):
    ...
    if log_format == "CLF":
        ...
        f.write('{ip:%s, dt:%s, tz:%s, vrb:%s, uri:%s, resp:%s, byt:%s}\n' % (ip,dt,tz,vrb,uri,resp,byt))
		#f.write('%s^%s^%s^%s^%s^%s^%s\n' % (ip,dt,tz,vrb,uri,resp,byt))
    elif log_format == "ELF": 
        pageId = random.randint(30, 3000)
    #    if pageId not in list:
    #        list.append(pageId)
    #        f.write('{"pageId" : "%s", "dt" : "%s"}\n' % (pageId,dt))
    ...
    if args.sleep:
        #time.sleep(args.sleep)
            time.sleep(float(random.gauss(1,0.1)))


# COMMAND ----------

df=spark.read.json('dbfs:/FileStore/faker_data/access_log.log')
